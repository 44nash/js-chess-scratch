...
function dragDrop(e) {
  ...
  if (correctGo) {
    if (takenByOpponent && valid) {
      // drop piece into square
      e.target.parentNode.append(draggedElement);

      //take over piece in that square //35.25
      let queue = e.target.parentNode;
      let elements = queue.getElementsByClassName('piece');
      ...
      if (
        ...
        (startId + width - 1 === targetId &&
          document.querySelector(`[square-id="${startId + width - 1}"]`)
            .firstChild) || // might need to remove first child
        (startId + width + 1 === targetId &&
          document.querySelector(`[square-id="${startId + width + 1}"]`)
            .firstChild) // might need to remove first child
      ) {
